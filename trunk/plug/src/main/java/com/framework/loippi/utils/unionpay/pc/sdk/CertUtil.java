...
public class CertUtil {
	...
	/** 基于Map存储多商户RSA私钥 */
	private final static Map<InputStream, KeyStore> certKeyStoreMap = new ConcurrentHashMap<InputStream, KeyStore>();
	 ...
	 * @deprecated
	 */
	public static void initSignCert(InputStream certFilePath, String certPwd) {
...
//		}
	 ...
	 * @param certPwd
	 */
	public static void loadRsaCert(InputStream certFilePath, String certPwd) {
...
//			return;
//		}
		List<InputStream> publicCertNames=SDKConfig.getConfig().getPublicCert();
		if (publicCertNames==null||publicCertNames.size()==0) {
		...
		try {
			cf = CertificateFactory.getInstance("X.509");
			for(int i=0;i<publicCertNames.size();i++){
				in=publicCertNames.get(i);
	...
	public static PrivateKey getSignCertPrivateKeyByThreadLocal(
			InputStream certPath, String certPwd) {
	...
	public static PrivateKey getSignCertPrivateKeyByStoreMap(InputStream certPath,
			String certPwd) {
	 ...
	 * @throws IOException 
	 */
	public static KeyStore getKeyInfo(InputStream pfxkeyfile, String keypwd,
...
//			fis = new FileInputStream(pfxkeyfile);
	 ...
	 * @return
	 */
	public static String getCertIdByThreadLocal(InputStream certPath, String certPwd) {
		// 初始化指定certPath和certPwd的签名证书容器
		initSignCert(certPath, certPwd);
		try {
			Enumeration<String> aliasenum = certKeyStoreLocal.get().aliases();
	...
	public static String getCertIdByKeyStoreMap(InputStream certPath, String certPwd) {
		if (!certKeyStoreMap.containsKey(certPath)) {
			// 缓存中未查询到,则加载RSA证书
			loadRsaCert(certPath, certPwd);
		}
