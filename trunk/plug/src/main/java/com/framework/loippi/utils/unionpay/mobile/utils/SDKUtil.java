 ...
 * =============================================================================
 */
package com.framework.loippi.utils.unionpay.mobile.utils;
...
public class SDKUtil {
    ...
    public static boolean sign(Map<String, String> data, String encoding) {
        ...
        // 设置签名证书序列号
        data.put(SDKConstants.param_certId, com.framework.loippi.utils.unionpay.pc.sdk.CertUtil.getSignCertId());
        ...
        try {
            ...
            byteSign = SecureUtil.base64Encode(SecureUtil.signBySoft(
                    com.framework.loippi.utils.unionpay.pc.sdk.CertUtil.getSignCertPrivateKey(), signDigest));
    ...
    public static boolean signByCertInfo(Map<String, String> data,
                                         String encoding, InputStream certPath, String certPwd) {
        if (isEmpty(encoding)) {
            encoding = "UTF-8";
        }
...
//		}
        ...
        data.put(SDKConstants.param_certId,
                com.framework.loippi.utils.unionpay.pc.sdk.CertUtil.getCertIdByKeyStoreMap(certPath, certPwd));
        // 将Map信息转换成key1=value1&key2=value2的形式
        String stringData = coverMap2String(data);
        /**
         * 签名\base64编码
        ...
        try {
            byte[] signDigest = SecureUtil.sha1X16(stringData, encoding);
            byteSign = SecureUtil.base64Encode(SecureUtil.signBySoft(
                    com.framework.loippi.utils.unionpay.pc.sdk.CertUtil.getSignCertPrivateKeyByStoreMap(certPath, certPwd),
                    signDigest));
            stringSign = new String(byteSign);
            // 设置签名域值
            data.put(SDKConstants.param_signature, stringSign);
    ...
    public static boolean validate(Map<String, String> resData, String encoding) {
        ...
        try {
            // 验证签名需要用银联发给商户的公钥证书.
            return SecureUtil.validateSignBySoft(com.framework.loippi.utils.unionpay.pc.sdk.CertUtil
    ...
    public static String encryptPin(String card, String pwd, String encoding) {
        return SecureUtil.EncryptPin(pwd, card, encoding, com.framework.loippi.utils.unionpay.pc.sdk.CertUtil
    ...
    public static String encryptCvn2(String cvn2, String encoding) {
        return SecureUtil.EncryptData(cvn2, encoding, com.framework.loippi.utils.unionpay.pc.sdk.CertUtil
                .getEncryptCertPublicKey());
    }
     ...
     * CVN2解密
     *
     * @param base64cvn2 加密后的CVN2值
     * @param encoding   字符编码
     * @return 解密后的数据
     */
    public static String decryptCvn2(String base64cvn2, String encoding) {
        return SecureUtil.DecryptedData(base64cvn2, encoding, com.framework.loippi.utils.unionpay.pc.sdk.CertUtil
    ...
    public static String encryptAvailable(String date, String encoding) {
        return SecureUtil.EncryptData(date, encoding, com.framework.loippi.utils.unionpay.pc.sdk.CertUtil
                .getEncryptCertPublicKey());
    }
     ...
     * 有效期解密.
     *
     * @param base64Date 有效期值
     * @param encoding   字符编码
     * @return 加密后的数据
     */
    public static String decryptAvailable(String base64Date, String encoding) {
        return SecureUtil.DecryptedData(base64Date, encoding, com.framework.loippi.utils.unionpay.pc.sdk.CertUtil
                .getSignCertPrivateKey());
    }
    ...
    public static String encryptPan(String pan, String encoding) {
        return SecureUtil.EncryptData(pan, encoding, com.framework.loippi.utils.unionpay.pc.sdk.CertUtil
                .getEncryptCertPublicKey());
    }
     ...
     * 卡号解密.
     *
     * @param base64Pan 如果卡号加密，传入返回报文中的卡号字段
     * @param encoding
     * @return
     */
    public static String decryptPan(String base64Pan, String encoding) {
        return SecureUtil.DecryptedData(base64Pan, encoding, com.framework.loippi.utils.unionpay.pc.sdk.CertUtil
                .getSignCertPrivateKey());
    }
    ...
    public static String encryptTrack(String trackData, String encoding) {
        return SecureUtil.EncryptData(trackData, encoding,
                com.framework.loippi.utils.unionpay.pc.sdk.CertUtil.getEncryptTrackCertPublicKey());
    }
