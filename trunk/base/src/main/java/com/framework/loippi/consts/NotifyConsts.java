package com.framework.loippi.consts;

/**
 * @author :ldq
 * @date:2020/4/9
 * @description:dubbo com.framework.loippi.consts
 */
public class NotifyConsts {
	/**
	 * admin回调路径
	 */
	//public final static String ADMIN_NOTIFY_FILE = "http://hqxsqs.natappfree.cc/admin_war_exploded";
	//public final static String ADMIN_NOTIFY_FILE = "http://glht.rdnmall.cn/admin";
	public final static String ADMIN_NOTIFY_FILE = "http://szleanshi.rdnmall.cn/admin";

	/**
	 * app回调路径
	 */
	//public final static String APP_NOTIFY_FILE = "http://df1af95fdf1ed929.natapp.cc/api";
	public final static String APP_NOTIFY_FILE = "http://www.rdnmall.com/api";
}
